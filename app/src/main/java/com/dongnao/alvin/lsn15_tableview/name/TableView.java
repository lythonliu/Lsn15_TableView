package com.dongnao.alvin.lsn15_tableview.name;
...
import com.dongnao.alvin.lsn15_tableview.BaseTableAdapter;
import com.dongnao.alvin.lsn15_tableview.R;
import com.dongnao.alvin.lsn15_tableview.Recycler;
...
public class TableView extends ViewGroup {
    ...
    public void scrollTo(int x, int y) {
        if (needRelayout) {
            ...
            scrollBy(x - sumArray(widths, 1, firstColumn) - scrollX, y - sumArray
                    (heights, 1, firstRow) - scrollY);
        }
    }
    ...
    public void scrollBy(int x, int y) {
        ...
        if (scrollX == 0) {
            ...
            while (!rowViewList.isEmpty() && getFilledWidth() - widths[firstColumn +
                    rowViewList.size()] >= width) {
                removeRight();
            }
...
computeHorizontalScrollExtent()
    ...
    protected int computeHorizontalScrollOffset() {
        ...
        final int maxHorizontalScrollOffset = width - widths[0] -
                computeHorizontalScrollExtent();
...
computeVerticalScrollExtent()
    ...
    protected int computeVerticalScrollOffset() {
        ...
        final int maxHorizontalScrollOffset = height - heights[0] -
                computeVerticalScrollExtent();
    ...
    private int getFilledWidth() {
        return widths[0] + sumArray(widths, firstColumn + 1, rowViewList.size()) -
                scrollX;
    }

    private int getFilledHeight() {
        return heights[0] + sumArray(heights, firstRow + 1, columnViewList.size()) -
                scrollY;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (adapter != null) {
            ...
            if (heightMode == MeasureSpec.AT_MOST || widthMode ==
        ...
        if (needRelayout || changed) {
            ...
            if (adapter != null) {
                ...
                for (int i = firstRow; i < rowCount && top < height; i++) {
                    ...
                    left = widths[0] - scrollX;
                    List<View> list = new ArrayList<View>();
                    for (int j = firstColumn; j < columnCount && left < width;
                         j++) {
                        right = left + widths[j + 1];
                        final View view = makeAndSetup(i, j, left, top,
                                right, bottom);
                        list.add(view);
                        left = right;
                    }
    ...
    private int scrollBounds(int desiredScroll, int firstCell, int sizes[], int viewSize) {
        if (desiredScroll == 0) {
            ...
            desiredScroll = Math.min(desiredScroll, Math.max(0, sumArray(sizes,
                    firstCell + 1, sizes.length - 1 - firstCell) + sizes[0] - viewSize));
        }
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (row == null || (row == -1 && column == -1)) {
            ...
            if (row == -1) {
                ...
                canvas.clipRect(0, heights[0], canvas.getWidth(),
                        canvas.getHeight());
            } else {
                canvas.clipRect(widths[0], heights[0], canvas.getWidth(),
                        canvas.getHeight());
            }
    ...
    private View obtainView(int row, int colmun, int width, int height) {
        ...
        view.measure(MeasureSpec.makeMeasureSpec
                (width,MeasureSpec.EXACTLY),MeasureSpec.makeMeasureSpec(height,MeasureSpec.EXACTLY));
    ...
    private class Flinger implements Runnable {
        ...
        void start(int initX, int initY, int initialVelocityX, int initialVelocityY, int
                maxX, int maxY) {
